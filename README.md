...
- `images`: `photosessions`, `banners`, `restoration`
...
### Групування кількох фото в один альбом

Якщо кілька фото — це один сет (наприклад, одна фотосесія), додай кожному з них
однакове поле `"group"` (будь-який унікальний ідентифікатор сету) і `"groupTitle"`
(назва, яку побачить відвідувач на картці). На сайті вони з'являться однією
карткою з позначкою кількості фото, а клік відкриє перегляд із гортанням
стрілками між усіма фото сету.

```json
{
  "id": "photosession-model-1",
  "category": "images",
  "subcategory": "photosessions",
  "group": "model-photosession",
  "groupTitle": "Модель — фотосесія",
  "title": "Кадр 1",
  "type": "image",
