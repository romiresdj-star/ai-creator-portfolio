...
function renderGrid(catId) {
  ...
  grid.innerHTML = items
    .map((item, i) => {
      const taxSub = PORTFOLIO_TAXONOMY.find((c) => c.id === item.category)
        ?.subcategories.find((s) => s.id === item.subcategory);
      const thumbSrc = item.type === "video" ? item.thumbnail : item.src;
      const isVideo = item.type === "video";

      return `
        <article class="work-card ${item.placeholder ? "is-placeholder" : ""}" data-index="${ALL_WORKS.indexOf(item)}">
          ...
          <div class="work-info">
            ...
            <span class="work-tag">${item.placeholder ? "Приклад" : taxSub?.label || ""}</span>
          </div>
        </article>
      `;
    })
  ...
  grid.querySelectorAll(".work-card").forEach((card) => {
    card.addEventListener("click", () => openLightbox(ALL_WORKS[card.dataset.index]));
  });
}
